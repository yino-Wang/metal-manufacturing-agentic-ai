...
public class AllocateMaterialsCommandService {
    ...
    public void allocateMaterials(AddJobMaterialsCommand addJobMaterialsCommand) {
        ...
        Material material = inventoryRepository.findAll().stream()
                .filter(m -> m.getName().equalsIgnoreCase(addJobMaterialsCommand.getMaterialName()))
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Material not found: " + addJobMaterialsCommand.getMaterialName()));

        long newQty = material.getQuantity() - addJobMaterialsCommand.getMaterialAmount();
